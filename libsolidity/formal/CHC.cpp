...
bool CHC::visit(ContractDefinition const& _contract)
{
	...
	m_constructorSummaryPredicate = createSymbolicBlock(constructorSort(), "summary_constructor_" + contractSuffix(_contract), PredicateType::ConstructorSummary, &_contract);
...
{
	auto implicitConstructorPredicate = createSymbolicBlock(smt::implicitConstructorSort(), "implicit_constructor_" + contractSuffix(_contract), PredicateType::ImplicitConstructor, &_contract);
...
void CHC::endVisit(FunctionDefinition const& _function)
{
	...
	else
	{
		...
		if (_function.isConstructor())
		{
			...
			auto constructorExit = createSymbolicBlock(constructorSort(), "constructor_exit_" + suffix, PredicateType::ConstructorSummary, m_currentContract);
...
void CHC::defineInterfacesAndSummaries(SourceUnit const& _source)
{
	for (auto const& node: _source.nodes())
		if (auto const* contract = dynamic_cast<ContractDefinition const*>(node.get()))
		{
			...
			m_interfaces[contract] = createSymbolicBlock(smt::interfaceSort(*contract), "interface_" + suffix, PredicateType::Interface, contract);
			m_nondetInterfaces[contract] = createSymbolicBlock(smt::nondetInterfaceSort(*contract), "nondet_interface_" + suffix, PredicateType::NondetInterface, contract);
...
{
	return smt::interface(*m_interfaces.at(&_contract), _contract, m_context);
}
...
{
	return smt::constructor(*m_constructorSummaryPredicate, _contract, m_context);
}
...
{
	m_errorPredicate = createSymbolicBlock(smt::arity0FunctionSort(), "error_target_" + to_string(m_context.newUniqueId()), PredicateType::Error);
...
smtutil::Expression CHC::predicate(Predicate const& _block)
{
	...
	case PredicateType::Interface:
		solAssert(m_currentContract, "");
		return smt::interface(_block, *m_currentContract, m_context);
	case PredicateType::ImplicitConstructor:
		solAssert(m_currentContract, "");
		return smt::implicitConstructor(_block, *m_currentContract, m_context);
	case PredicateType::ConstructorSummary:
		solAssert(m_currentContract, "");
		return smt::constructor(_block, *m_currentContract, m_context);
	...
	case PredicateType::FunctionBlock:
		solAssert(m_currentFunction, "");
		return smt::functionBlock(_block, *m_currentFunction, m_currentContract, m_context);
