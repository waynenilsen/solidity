...
SortPointer constructorSort(ContractDefinition const& _contract)
{
	...
	return make_shared<smtutil::FunctionSort>(
		vector<smtutil::SortPointer>{smtutil::SortProvider::uintSort} + stateSorts(_contract),
		smtutil::SortProvider::boolSort
	);
}
...
SortPointer functionSort(FunctionDefinition const& _function, ContractDefinition const* _contract)
{
	...
	return make_shared<smtutil::FunctionSort>(
		vector<SortPointer>{smtutil::SortProvider::uintSort} +
			varSorts +
			inputSorts +
			varSorts +
			inputSorts +
...
{
	auto fSort = dynamic_pointer_cast<smtutil::FunctionSort>(functionSort(_function, _contract));
	solAssert(fSort, "");
